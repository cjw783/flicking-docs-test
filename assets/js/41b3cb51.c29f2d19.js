...
}`:""})`)});`:"",x=[...c.map(e=>[`{ ${e[0]} }`,"@egjs/flicking-plugins"]),...h],$=Object.keys(n).map(e=>{let i=n[e];return`flicking.on("${e}", ${i})
`}).join(""),k=i.filter(e=>e.isSlot);return(0,s.jsxs)(s.Fragment,{children:[(0,s.jsx)(t.A,{className:"language-html",title:"html",children:`<div id="flick" class="flicking-viewport${u&&` ${u}`}${!1===e.horizontal?" vertical":""}">
  <div class="flicking-camera">
    ${i.filter(e=>!e.isSlot).map(e=>`<${e.tag}${d(e)}${a(e)}>${e.content}</${e.tag}>`).join("\n    ")}
  </div>${k.length?`
  ${k.map(e=>`<${e.tag}${d(e)}${a(e)}>${e.content}</${e.tag}>`).join("\n  ")}`:""}
</div>${o?"\n"+o.map(e=>`<${e.tag}${d(e)}${a(e)}>${e.content}</${e.tag}>`).join("\n"):""}`}),(0,s.jsx)(t.A,{className:"language-js",title:"js",children:`${p(x)}

${m}${j}${f}${$}`.trim()})]})},g=({options:e,panels:i,events:n={},methods:l={},plugins:c,siblings:r,imports:o=[],viewportClass:u=""})=>{let g=c?`
...
export default class DemoComponent extends Component {${g}
  public render() {
    return ${r?"<>\n    ":""}<Flicking${u&&` className="${u}"`}${e?` ${Object.keys(e).map(i=>`${i}=${"string"==typeof e[i]?`"${e[i]}"`:`{${e[i]}}`}`).join(" ")}`:""}${c?" plugins={this._plugins}":""}${$}>
  ...
  </template>`:"",$=[...(c??[]).map(e=>[`{ ${e[0]} }`,"@egjs/flicking-plugins"]),...u],k=Object.keys(l),y=k.length>0?`,
  methods: {
