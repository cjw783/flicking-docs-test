...
`;function d({code:e,html:i,template:n="react",dependencies:l={},files:d={},css:p=""}){var h;let g,u,m;return(0,s.jsx)(t.OZ,{template:"vue3"===n?"vue":"vanilla"===n?"vanilla":"react",files:(m={"/styles.css":{code:u=p?`${a}
${p}`:a},...d},"react"===n?{"/App.tsx":{code:e},"/index.js":{code:o,hidden:!0},...m}:"vue3"===n?{"/src/App.vue":{code:e},...m}:{"/src/index.js":{code:e},"/index.js":{code:function(e){let i=e.split("\n"),n=[],s=[],t=!1;for(let e of i)t?s.push(e):e.trimStart().startsWith("import ")||""===e.trim()?n.push(e.replace('"../styles.css"','"./styles.css"')):(t=!0,s.push(e));for(;s.length>0&&""===s[s.length-1].trim();)s.pop();return[...n,"","const _run = () => {",...s.map(e=>`  ${e}`),'  document.querySelectorAll(".flicking-viewport").forEach(el => { el.style.opacity = "1"; });',"};","",'if (document.readyState === "loading") {','  document.addEventListener("DOMContentLoaded", _run);',"} else {","  _run();","}"].join("\n")}(e),hidden:!0},"/styles.css":{code:u},"/index.html":{code:(g="<style>.flicking-viewport{opacity:0}</style>",(h=i||c).includes("</head>")?h.replace("</head>",`${g}
</head>`):g+h)},...d}),customSetup:{dependencies:{...r[n],...l}},options:{showLineNumbers:!0,editorHeight:400,externalResources:[],visibleFiles:"react"===n?["/App.tsx","/styles.css"]:"vue3"===n?["/src/App.vue","/styles.css"]:["/src/index.js","/index.html","/styles.css"],activeFile:"react"===n?"/App.tsx":"vue3"===n?"/src/App.vue":"/src/index.js"}})}},72722(e,i,n){n.d(i,{A:()=>x});var s=n(65723),t=n(34622),l=n(7210),r=n(78863);n(22155);let c=(e,i='"')=>"string"==typeof e?`${i}${e}${i}`:e,o=(e,i="class")=>e.class?` ${i}="${e.class}"`:"",a=(e,i=!1)=>e.style?i?` style={{ ${Object.keys(e.style).map(i=>`${i}: ${c(e.style[i])}`).join(", ")} }}`:` style="${Object.keys(e.style).map(i=>`${i}: ${e.style[i]};`).join(" ")}"`:"",d=e=>`${Object.keys(e).map(i=>`${i}: ${c(e[i],"'")}`).join(", ")}`,p=(e,{prefix:i,includeFlicking:n=!0}={})=>{let s=n?[["Flicking",`@egjs/${i?`${i}-`:""}flicking`]]:[];return e&&s.push(...e),`${s.map(e=>Array.isArray(e)?`import ${e[0]} from "${e[1]}";`:`import "${e}";`).join("\n")}`},h=e=>`${e.map(e=>`new ${e[0]}(${"object"==typeof e[1]?`{ ${d(e[1])} }`:e[1]??""}`).join(", ")})`,g=({options:e,panels:i,events:n={},methods:l={},plugins:r=[],siblings:d=[],imports:h=[],viewportClass:g=""})=>{let u=e=>`${Object.keys(e).map(i=>`${i}: ${c(e[i])}`).join(",\n  ")}`,m=Object.keys(l).map(e=>`const ${e} = ${l[e]};
`).join(""),x=e?`const flicking = new Flicking("#flick", {
  ${u(e)}
});

`:'const flicking = new Flicking("#flick")',j=r.length>0?`flicking.addPlugins(${r.map(e=>`new ${e[0]}(${e[1]?`{
  ${"object"==typeof e[1]?u(e[1]):e[1]??""}
}`:""})`)});`:"",f=[...r.map(e=>[`{ ${e[0]} }`,"@egjs/flicking-plugins"]),...h],$=Object.keys(n).map(e=>{let i=n[e];return`flicking.on("${e}", ${i})
`}).join(""),v=i.filter(e=>e.isSlot);return(0,s.jsxs)(s.Fragment,{children:[(0,s.jsx)(t.A,{className:"language-html",title:"html",children:`<div id="flick" class="flicking-viewport${g&&` ${g}`}${!1===e.horizontal?" vertical":""}">
...
${m}${x}${j}${$}`.trim()})]})},u=({options:e,panels:i,events:n={},methods:l={},plugins:r,siblings:c,imports:d=[],viewportClass:g=""})=>{let u=r?`
  private _plugins = [${h(r)}];
`:"",m=i.filter(e=>e.isSlot),x=[["{ Component }","react"],[m.length?"Flicking, { ViewportSlot }":"Flicking","@egjs/react-flicking"]],j=Object.keys(l).map(e=>`
const ${e} = ${l[e]};`).join("");j&&(j+="\n"),r&&x.push([`{ ${r.map(e=>e[0])} }`,"@egjs/flicking-plugins"]),x.push(...d);let f=m.length?`
      <ViewportSlot>
        ${m.map(e=>`<${e.tag}${o(e,"className")}${a(e,!0)}>${e.content.replace(/class/g,"className")}</${e.tag}>`).join("\n        ")}
      </ViewportSlot>`:"",$=Object.keys(n).map(e=>` on${e[0].toUpperCase()+e.slice(1)}={${n[e]}}`).join("");return(0,s.jsx)(t.A,{className:"language-jsx",title:"DemoComponent.jsx",children:`${p(x,{includeFlicking:!1})}
${j}
export default class DemoComponent extends Component {${u}
  public render() {
    return ${c?"<>\n    ":""}<Flicking${g&&` className="${g}"`}${e?` ${Object.keys(e).map(i=>`${i}=${"string"==typeof e[i]?`"${e[i]}"`:`{${e[i]}}`}`).join(" ")}`:""}${r?" plugins={this._plugins}":""}${$}>
...
}`})},m=({options:e,panels:i,events:n={},methods:l={},plugins:r,siblings:c=[],imports:g=[],viewportClass:u=""})=>{let m=d(e),x=i.filter(e=>e.isSlot),j=r?`,
  data() {
    return {
      plugins: [${h(r)}]
    }
  ...
  <template #viewport>
    ${x.map(e=>`<${e.tag}${o(e)}${a(e)}>${e.content}</${e.tag}>`).join("\n    ")}
  </template>`:"",$=[...(r??[]).map(e=>[`{ ${e[0]} }`,"@egjs/flicking-plugins"]),...g],v=Object.keys(l),k=v.length>0?`,
  methods: {
...
}`})]})},x=({js:e,react:i,vue3:n,style:c,...o})=>(0,s.jsxs)("div",{children:[(0,s.jsxs)(r.A,{groupId:"cfc",defaultValue:"js",children:[(0,s.jsx)(l.A,{value:"react",label:"React",children:i||(0,s.jsx)(u,{...o})}),(0,s.jsx)(l.A,{value:"vue3",label:"Vue@3",children:n||(0,s.jsx)(m,{...o})}),(0,s.jsx)(l.A,{value:"js",label:"JavaScript",children:e||(0,s.jsx)(g,{...o})})]}),c&&(0,s.jsx)(t.A,{className:"language-css",title:"style",children:`${c}`})]})}}]);