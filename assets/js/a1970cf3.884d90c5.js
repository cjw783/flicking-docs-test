...
</div>${p?"\n"+p.map(e=>`<${e.tag}${r(e)}${d(e)}>${e.content}</${e.tag}>`).join("\n"):""}`}),(0,s.jsx)(n.A,{className:"language-js",title:"js",children:`${o($)}

${u}${j}${x}${f}`.trim()})]})},m=({options:e,panels:i,events:l={},methods:t={},plugins:a,siblings:c,imports:p=[],viewportClass:h=""})=>{let m=a?`
...
const ${e} = ${t[e]};`).join("");x&&(x+="\n"),a&&j.push([`{ ${a.map(e=>e[0])} }`,"@egjs/flicking-plugins"]),j.push(...p);let $=u.length?`
      <ViewportSlot>
        ${u.map(e=>`<${e.tag}${r(e,"className")}${d(e,!0)}>${e.content.replace(/class/g,"className")}</${e.tag}>`).join("\n        ")}
      </ViewportSlot>`:"",f=Object.keys(l).map(e=>` on${e[0].toUpperCase()+e.slice(1)}={${l[e]}}`).join("");return(0,s.jsx)(n.A,{className:"language-jsx",title:"DemoComponent.jsx",children:`${o(j,{includeFlicking:!1})}
${x}
export default class DemoComponent extends Component {${m}
...
}`})},u=({options:e,panels:i,events:l={},methods:t={},plugins:a,siblings:c=[],imports:h=[],viewportClass:m=""})=>{let u=p(e),j=i.filter(e=>e.isSlot),x=a?`,
  data() {
    return {
      plugins: [${g(a)}]
    }
  ...
  </template>`:"",f=[...(a??[]).map(e=>[`{ ${e[0]} }`,"@egjs/flicking-plugins"]),...h],v=Object.keys(t),k=v.length>0?`,
  methods: {
...
${c.map(e=>`<${e.tag}${r(e)}${d(e)}>${e.content}</${e.tag}>`).join("\n")}`:""}`}),(0,s.jsx)(n.A,{className:"language-js",title:"script",children:`${o(f,{prefix:"vue3"})}
