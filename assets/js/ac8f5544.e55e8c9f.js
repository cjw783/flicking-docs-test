...
`;function o({code:e,html:i,template:s="react",dependencies:n={},files:o={},css:p=""}){var g;let m,h,u;return(0,l.jsx)(t.OZ,{template:"vue3"===s?"vue":"vanilla"===s?"vanilla":"react",files:(u={"/styles.css":{code:h=p?`${d}
${p}`:d},...o},"react"===s?{"/App.tsx":{code:e},"/index.js":{code:r,hidden:!0},...u}:"vue3"===s?{"/src/App.vue":{code:e},...u}:{"/src/index.js":{code:e},"/index.js":{code:function(e){let i=e.split("\n"),s=[],l=[],t=!1;for(let e of i)t?l.push(e):e.trimStart().startsWith("import ")||""===e.trim()?s.push(e.replace('"../styles.css"','"./styles.css"')):(t=!0,l.push(e));for(;l.length>0&&""===l[l.length-1].trim();)l.pop();return[...s,"","const _run = () => {",...l.map(e=>`  ${e}`),'  document.querySelectorAll(".flicking-viewport").forEach(el => { el.style.opacity = "1"; });',"};","",'if (document.readyState === "loading") {','  document.addEventListener("DOMContentLoaded", _run);',"} else {","  _run();","}"].join("\n")}(e),hidden:!0},"/styles.css":{code:h},"/index.html":{code:(m="<style>.flicking-viewport{opacity:0}</style>",(g=i||a).includes("</head>")?g.replace("</head>",`${m}
</head>`):m+g)},...o}),customSetup:{dependencies:{...c[s],...n}},options:{showLineNumbers:!0,editorHeight:400,externalResources:[],visibleFiles:"react"===s?["/App.tsx","/styles.css"]:"vue3"===s?["/src/App.vue","/styles.css"]:["/src/index.js","/index.html","/styles.css"],activeFile:"react"===s?"/App.tsx":"vue3"===s?"/src/App.vue":"/src/index.js"}})}},72722(e,i,s){s.d(i,{A:()=>f});var l=s(65723),t=s(34622),n=s(7210),c=s(78863);s(22155);let a=(e,i='"')=>"string"==typeof e?`${i}${e}${i}`:e,r=(e,i="class")=>e.class?` ${i}="${e.class}"`:"",d=(e,i=!1)=>e.style?i?` style={{ ${Object.keys(e.style).map(i=>`${i}: ${a(e.style[i])}`).join(", ")} }}`:` style="${Object.keys(e.style).map(i=>`${i}: ${e.style[i]};`).join(" ")}"`:"",o=e=>`${Object.keys(e).map(i=>`${i}: ${a(e[i],"'")}`).join(", ")}`,p=(e,{prefix:i,includeFlicking:s=!0}={})=>{let l=s?[["Flicking",`@egjs/${i?`${i}-`:""}flicking`]]:[];return e&&l.push(...e),`${l.map(e=>Array.isArray(e)?`import ${e[0]} from "${e[1]}";`:`import "${e}";`).join("\n")}`},g=e=>`${e.map(e=>`new ${e[0]}(${"object"==typeof e[1]?`{ ${o(e[1])} }`:e[1]??""}`).join(", ")})`,m=({options:e,panels:i,events:s={},methods:n={},plugins:c=[],siblings:o=[],imports:g=[],viewportClass:m=""})=>{let h=e=>`${Object.keys(e).map(i=>`${i}: ${a(e[i])}`).join(",\n  ")}`,u=Object.keys(n).map(e=>`const ${e} = ${n[e]};
...
${u}${f}${j}${x}`.trim()})]})},h=({options:e,panels:i,events:s={},methods:n={},plugins:c,siblings:a,imports:o=[],viewportClass:m=""})=>{let h=c?`
  private _plugins = [${g(c)}];
`:"",u=i.filter(e=>e.isSlot),f=[["{ Component }","react"],[u.length?"Flicking, { ViewportSlot }":"Flicking","@egjs/react-flicking"]],j=Object.keys(n).map(e=>`
const ${e} = ${n[e]};`).join("");j&&(j+="\n"),c&&f.push([`{ ${c.map(e=>e[0])} }`,"@egjs/flicking-plugins"]),f.push(...o);let $=u.length?`
      <ViewportSlot>
        ${u.map(e=>`<${e.tag}${r(e,"className")}${d(e,!0)}>${e.content.replace(/class/g,"className")}</${e.tag}>`).join("\n        ")}
      </ViewportSlot>`:"",x=Object.keys(s).map(e=>` on${e[0].toUpperCase()+e.slice(1)}={${s[e]}}`).join("");return(0,l.jsx)(t.A,{className:"language-jsx",title:"DemoComponent.jsx",children:`${p(f,{includeFlicking:!1})}
${j}
...
}`})},u=({options:e,panels:i,events:s={},methods:n={},plugins:c,siblings:a=[],imports:m=[],viewportClass:h=""})=>{let u=o(e),f=i.filter(e=>e.isSlot),j=c?`,
  data() {
    return {
      plugins: [${g(c)}]
    }
...
}`})]})},f=({js:e,react:i,vue3:s,style:a,...r})=>(0,l.jsxs)("div",{children:[(0,l.jsxs)(c.A,{groupId:"cfc",defaultValue:"js",children:[(0,l.jsx)(n.A,{value:"react",label:"React",children:i||(0,l.jsx)(h,{...r})}),(0,l.jsx)(n.A,{value:"vue3",label:"Vue@3",children:s||(0,l.jsx)(u,{...r})}),(0,l.jsx)(n.A,{value:"js",label:"JavaScript",children:e||(0,l.jsx)(m,{...r})})]}),a&&(0,l.jsx)(t.A,{className:"language-css",title:"style",children:`${a}`})]})}}]);